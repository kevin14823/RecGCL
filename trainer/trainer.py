...
class Trainer(AbstractTrainer):
    ...
    def _valid_epoch(self, valid_data, show_progress=False):
        ...
        valid_result = self.evaluate(
            valid_data, load_best_model=False, show_progress=show_progress, test=False
        )
    ...
    @torch.no_grad()
    def evaluate(
            self, eval_data, load_best_model=True, model_file=None, show_progress=False, test=False
        ):
        r"""Evaluate the model based on the eval data.
        ...
        Args:
            ...
            show_progress (bool): Show the progress of evaluate epoch. Defaults to ``False``.
            noise_pairs (iterable, optional): A collection of noise pairs for alignment computation.
        ...
        # ----------------------------------------------------------------
        # 確保 noise_pairs 存在，如果沒有則從 trainer 讀取
        # ----------------------------------------------------------------
        noise_pairs = getattr(self, "noise_pairs", None)

        if noise_pairs is not None and len(noise_pairs) > 0:
            self.logger.info(f"Received {len(noise_pairs) if noise_pairs else 0} noise pairs for evaluation.")
            alignment_score = self.compute_alignment_on_noise(self.model, noise_pairs)
            self.logger.info("Alignment on noise pairs: {:.4f}".format(alignment_score))

        self.wandblogger.log_eval_metrics(result, head="eval")
        
        return result
    
    def compute_alignment_on_noise(self, model, noise_pairs):
        # forward 拿到最終 embedding
        user_all_embeddings, item_all_embeddings, _ = model.forward()

        distances = []
        for (u, i) in noise_pairs:
            u_emb = user_all_embeddings[u]
            i_emb = item_all_embeddings[i]
            dist = torch.norm(u_emb - i_emb, p=2)  # L2 norm
            distances.append(dist.item())
            if u >= len(user_all_embeddings) or i >= len(item_all_embeddings):
                self.logger.info(f"Skipping out-of-range noise pair: ({u}, {i})")
            continue
