...
class NCLTrainer(Trainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            # 判斷是否為該 epoch 的「最後一個 batch」
            is_last_batch = (batch_idx == len(iter_data) - 1)

            with amp.autocast(enabled=self.enable_amp):
                # 把 is_last_batch 傳給 calculate_loss，控制是否計算 DE/MAD
...
class RecGCLTrainer(Trainer):
    def __init__(self, config, model):
        super(RecGCLTrainer, self).__init__(config, model)
